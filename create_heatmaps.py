...
from dataset.datasets import WSIDataset
...
def run(args):
    ...
    for patch_feature, _, case_id in dataset:
        ...
        with torch.no_grad():
            ...
            mil_attention = model.module.gated_mil.mil.forward(patch_feature,
                                                               attention_only=True).cpu().numpy().reshape(-1)
        print(f"attention: {mil_attention.shape}")
        ...
        heatmap = create_heatmap(str(coord_filepath), mil_attention, slide_level=args.slide_level, contours=contours)
...
def main():
    ...
    parser.add_argument('--data_csv', type=str,
                        ...
                        help='')
    parser.add_argument('--coord_dir', type=str, default='/dir/to/coord/dir',
                        help='')
