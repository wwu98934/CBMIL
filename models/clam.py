...
class CLAM_SB(nn.Module):
    def __init__(self, gate=True, size_arg="small", dropout=False, k_sample=8, n_classes=2,
        ...
        # Classifier
        self.classifiers = nn.Linear(size[1], n_classes)  # bag classifier
        instance_classifiers = [nn.Linear(size[1], 2) for _ in range(n_classes)]
        self.instance_classifiers = nn.ModuleList(instance_classifiers)  # instance_classifiers
    ...
    def forward(self, bag, label=None, attention_only=False):
        # device = h.device
        if len(bag.shape) == 3 and bag.shape[0] == 1:
            bag = bag.squeeze(0)
        assert len(bag.shape) == 2, f"h.shape: {bag.shape}"
        # print(f"bag: {bag.shape}")
        A, h = self.attention_net(bag)  # NxK
        # print(f"A: {A.shape} | h: {h.shape}")
        A = torch.transpose(A, 1, 0)  # KxN
        if attention_only:
            return A
        A = F.softmax(A, dim=1)  # softmax over N
        M = torch.mm(A, h)
        outputs = self.classifiers(M)

        if label is not None:
            total_inst_loss = 0.0
            all_preds = []
            all_targets = []
            inst_labels = F.one_hot(label, num_classes=self.n_classes).squeeze()  # binary label
            for i in range(len(self.instance_classifiers)):  # loop num_classes
            ...
            return outputs, M, total_inst_loss
...
class CLAM_MB(CLAM_SB):
    ...
    def forward(self, h, label=None, attention_only=False):
        device = h.device
        ...
        if label is not None:
            ...
            if self.subtyping:
                total_inst_loss /= len(self.instance_classifiers)
            return logits, M, total_inst_loss
